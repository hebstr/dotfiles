...
invocations_filter='.timestamp[0:10] as $d | .message.content
  | (if type == "string" then [.] else [.[]? | select(.type == "text") | .text] end
      | .[]
      | capture("<command-name>/(?<k>[^<]+)</command-name>\\s*<command-args>(?<a>[^<]*)")?
      | "\($d)\t\(.k)\t\((.a | split("\n")[0]) // "")"),
    (if type == "array" then
...
# shellcheck disable=SC2016
bash_filter='.timestamp[0:10] as $d | .message.content
  | if type == "array" then

...
La conversation se mène dans la langue de l'utilisateur ; le registre suit la langue des notes du projet.
...
Un registre existant ne s'écrase jamais : le script refuse, et `--force` ne se passe que sur instruction explicite. Un répertoire ignoré par git n'a pas de filet.
...
Le résumé du script donne le compte des commentaires, des points, des réponses, des points sans ancre et, avec `--source`, des extraits non retrouvés ou ambigus.
Chaque extrait non retrouvé ou ambigu se résout en lisant la source : un extrait court se complète par la phrase qui le porte, un extrait introuvable se situe par sa section et le dit.
...
Chacune reçoit, à sa place, « écartée au tri » avec la date, sans reformulation ni proposition.
...
3. **La question de décision** : garder, modifier ou refuser.
...
Vocabulaire de décision : « proposition validée », « sauté en revue », avec la mention de ce que l'utilisateur fera lui-même quand il le dit, « reporté », avec l'endroit où la tâche est consignée.

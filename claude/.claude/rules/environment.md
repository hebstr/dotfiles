...
| opencode | Terminal AI coding agent (`~/.npm-global/bin/opencode`, npm package `opencode-ai`, so the `npm` module of `sys-update` carries it). Installed 2026-09-21 to be evaluated against a local model served by `llama-server` on `ju-TP2`, never against a paid API from here: its provider is declared in the `opencode` stow package (`.config/opencode/opencode.json`) and points at `http://127.0.0.1:8080/v1`, which is an SSH forward to that machine. It needs the forward standing and the remote server running, which `llama-session` provides, and a context of at least 32k, its baseline request weighing about 23800 tokens; design, measurements and the rest of the settings in `.claude/DESIGN-GPU-REMOTE.md`. The same `opencode.json` carries the harness that opens the Claude Code profile to it (`skills.paths` fed by `opencode-skills-sync`, read access to `~/.claude` without write access, skills denied or gated by `permission.skill`), designed in `.claude/DESIGN-OPENCODE-HARNESS.md` |

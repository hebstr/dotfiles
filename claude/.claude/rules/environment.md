...
The same `~/.npmrc` carries `allow-scripts=opencode-ai`, set 2026-09-21: npm 11.19 lists every install script missing from that allowlist as a "Phase 1 advisory" warning and still runs it, but a later phase blocks it, and `opencode-ai` needs its `postinstall` to link the platform binary into `bin/opencode.exe`. A package that newly shows up in that warning gets reviewed and added the same way, since the setting lives in a file that is never stowed and has to be set again on each machine.
...
| opencode | Terminal AI coding agent (`~/.npm-global/bin/opencode`, npm package `opencode-ai`, so the `npm` module of `sys-update` carries it). Installed 2026-09-21 to be evaluated against a local model served by `llama-server` on `ju-TP2`, never against a paid API from here: its provider is declared in the `opencode` stow package (`.config/opencode/opencode.json`) and points at `http://127.0.0.1:8080/v1`, which is an SSH forward to that machine. It needs the forward standing and the remote server running, which `llama-session` provides, and a context of at least 32k, its baseline request weighing about 23800 tokens; design, measurements and the rest of the settings in `.claude/DESIGN-GPU-REMOTE.md`. The same `opencode.json` carries the harness that opens the Claude Code profile to it (`instructions` loading each project's `.claude/CLAUDE.md` and `.claude/memory/MEMORY.md`, `skills.paths` fed by `opencode-skills-sync`, read access to `~/.claude`, skills denied or gated by `permission.skill`, `bash` on ask outside a read-and-gate allowlist), with a global `AGENTS.md` and a `plugins/claude-hooks.ts` that refuses any edit resolving under the Claude Code profile and runs the Claude Code hooks `prose-lint-pretool.sh` and `format-on-edit.sh` on the others, and strips the global profile that the `instructions` walk reaches outside a git repository or inside `~/dotfiles/claude`, designed in `.claude/DESIGN-OPENCODE-HARNESS.md`; the reproducible procedure and its replayable checks are `_meta/notes/opencode-harness-setup.md` |
| llama-session | Brings up and tears down one inference session on the GPU host (`~/.local/bin/llama-session`, `bin` stow package): it reuses a `llama-server` already running on `ju-TP2` or starts one, forwards its port to `127.0.0.1`, waits on `/health` while telling a slow load from a dead start, and on exit stops only the server it started itself. `--status`, `--stop`, `--keep`, and `LLAMA_MODEL_REPO` / `LLAMA_MODEL_FILE` / `LLAMA_CTX` / `LLAMA_PORT` and their siblings for the overrides. This is what a session of `opencode` runs first; design in `.claude/DESIGN-GPU-REMOTE.md` |
| opencode-skills-sync | Exposes the skills of the enabled Claude Code plugins to opencode (`~/.local/bin/opencode-skills-sync`, `bin` stow package): reads `installed_plugins.json`, `enabledPlugins` and each plugin's `marketplace.json`, and rebuilds one symlink per enabled skill in `~/.local/share/opencode-claude-skills/`, which `opencode.json` declares under `skills.paths`. The farm is swapped in only once complete, and any drift in those undocumented files fails the run rather than emptying it. `claude-plugins-update` calls it whenever opencode is installed, so `sys-update claude-plugins` resyncs it, while a plugin changed through Claude Code itself (auto-update, `/plugin`) leaves the farm stale until a manual run (`.claude/DEFERRED.md`, 2026-09-21); design in `.claude/DESIGN-OPENCODE-HARNESS.md` |
| llama-update | Installs a llama.cpp CUDA build on the machine that runs it, which is `ju-TP2` (`~/.local/bin/llama-update`, `bin` stow package, run there or as `ssh ju-TP2 .local/bin/llama-update`). One tree per build in `~/.local/opt/llama.cpp-<tag>/` behind the `~/.local/opt/llama.cpp` symlink that `llama-session` names; the link moves only once the new build lists `CUDA0`, the previous build stays for rollback (`llama-update <old tag>` repoints without downloading), older ones are removed, `--list` shows them. Without a tag it takes the `nightly-tag.txt` of the latest `v*` release, or the first later build that ships the CUDA archives when that one does not (upstream skips the CUDA build on about a quarter of tags, the stable one included on 2026-09-21), found through `gh`; never the latest daily `b*` build, and never below the active build, a downgrade taking an explicit tag. Both archives are probed before any download. The switch gate only proves a CUDA device is listed: a build can still drop a setting the chain relies on (`b10969` ignores `--reasoning-budget 0`), so after an update, bring up `llama-session` and send one request before relying on it. Deliberately not a `sys-update` module: upstream cuts a build almost daily and an unattended one can break the opencode chain. Design in `.claude/DESIGN-GPU-REMOTE.md` |

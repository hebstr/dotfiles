...
R and Python chunks inside a `.qmd` follow their own language rules: see `rules/r.md` and `rules/python.md` (idioms, and the `air`/`ruff` format+lint gate). `panache` applies the format+lint half in place: `panache format <file>.qmd` reformats R chunk bodies through `air`, and `panache lint <file>.qmd` reports `jarl`/`ruff` diagnostics against the chunk's own line numbers. Prefer it over a scratch-file round trip for R; a Python chunk still owes the round trip, its formatting half never firing (below).
**Chunk formatting never sees the project's `air.toml`**: panache writes each chunk to `/tmp/panache-<uuid>.r` and calls `air format` on that path, so air walks up from `/tmp` and finds no project config (measured 2026-09-11 by interposing a shim on `air` in the PATH).
Its own `line-width` is not passed down either, `-o line-width=100` and a project `panache.toml` at 120 both leaving the chunk at 80.
So chunk width comes from the user-level `~/.config/air/air.toml` (see `rules/r.md`), and a project whose `air.toml` departs from it gets its chunks formatted at a width its scripts do not use.
The delegation only became live in panache 3.0.0 (2026-07-20, `fix(formatter): match [formatters] keys by language alias`): 2.58.0 left R chunks untouched, which is why a stale recollection of "panache does not reformat my code" is worth re-testing rather than trusting.
Python chunks are not delegated at all despite `python = "ruff"` in the config, the shim never firing on a `{python}` chunk and `z=[1,  2]` surviving a format pass. It delegates formatters and linters only, so Python's type-checking step has no panache equivalent: a substantive Python chunk still owes the `pyrefly check` of `rules/python.md`, run against a scratch copy of the chunk body. Where panache is unavailable, copy a substantive chunk body (same threshold as the file-edit gate in `CLAUDE.md`) to a scratch `.R`/`.py` file, run the language gate from `rules/r.md`/`rules/python.md`, then paste the formatted result back. Either way, re-render *without* `--no-execute` afterwards to confirm the chunk still executes: reformatting can shift the chunk's indentation or disturb the fence boundaries, and a `--no-execute` pass validates YAML but not chunk execution, so it would give a false pass on exactly that breakage. Skip for trivial chunks (a single plot or `mutate` call). Do not duplicate those conventions here.

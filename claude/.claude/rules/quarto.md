...
- On a user-facing `.qmd`, `prose-lint <file>` is the mandatory mechanical check (dashes); `panache` handles the soft-wrap reflow (`wrap = "semantic"`); `/workflow:write` is optional deeper polish and anti-AI-slop review. Both apply to authored documents, not to this rules file.

...
**Output mode is auto-detected:** TTY stdout → padded columns with ANSI colors (each row colored by tier: P0 bold red, P1 yellow, P2 default; status appears as `[bracketed tag]` after the item, omitted if empty; "Fichiers vides" / "Fichiers non conformes" section headers rendered in cyan). Pipe/redirect stdout → markdown table (`Prio | Projet | Item | Statut`) suitable for `grep`, `glow`, or `> file.md`. `NO_COLOR=1` disables colors in TTY mode while keeping padding.

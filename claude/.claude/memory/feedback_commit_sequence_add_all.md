...
**Where the format lives (decided 2026-09-21):** the rendering and staging format belongs to the Git section of `~/.claude/CLAUDE.md` ("Every commit suggestion carries its message"); `skills/commit/SKILL.md` holds the procedure. Putting the format in the skill alone was ruled out: the skill carries `disable-model-invocation: true`, so it never loads for a commit suggested outside `/commit`, which is where most inconsistent proposals came from. The format agreed: one fenced block tagged `bash` per commit, holding its staging command and the full `git commit -m "<header>"` line, never the header alone. Open point: that the Claude Code terminal colorizes a `bash`-tagged fence is assumed, not verified.

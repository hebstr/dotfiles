...
**How to apply:** if a future task suggests "consolidate litrev into hebstr" or "add litrev as a hebstr plugin," push back with these three reasons before doing any work. Audit trail of the rollback is in `git log` (commits `2c911ca import litrev` then `a3836d7 revert litrev` on `claude-code-plugins` main, 2026-04-26). Marketplace mechanics learned during the rollback are in `reference_claude_code_marketplace_mechanics.md`.

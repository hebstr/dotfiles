...
**How to apply:** in any future config audit, code-review, or audit pass over `~/.claude/` config artifacts, skip the four live categories of suggestion (1, 2, 4, 5) immediately. They are recurring false positives. Real findings still apply: leaked credentials in committed settings, hooks that fail open and corrupt files, scope guards that demonstrably don't gate what they claim to gate, structural drift between CLAUDE.md and `rules/*.md` content, and logic bugs in hook scripts that produce wrong results within the script's stated contract.

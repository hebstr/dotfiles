...
- [feedback_review_severity_claude_config.md](feedback_review_severity_claude_config.md) — Calibrate `/health` and audits of `~/.claude` harness config (settings.json, hooks, CLAUDE.md): skip PWD-guard expansion, rg-in-hook-scripts, message-aware notifications, WebFetch allow-list expansion (4 recurring false positives)

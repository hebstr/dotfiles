...
- In `sys-cleanup`, an unguarded call to an optional dependency that aborts the rest of the run under `set -e` when the dependency is installed by default and the failure is loud (`pak::cache_clean()` with no pak check: `rig add` installs pak unless `--without-pak`). `sys-cleanup` runs on the workstation only, not on the multi-user servers of [[project_rv_install_deployment]], so absent-dependency triggers borrowed from that server context do not apply to it (walkthrough 2026-09-13)
...
- A missing `command -v syncthing` guard on `sys-update`'s `update_syncthing`, or a first-install path in `syncthing-update` (misleading "keeping the installed key" wording when no keyring exists yet). The app-presence guard of `positron`/`anki`/`libreoffice` exists to keep desktop apps off WSL, and Syncthing is wanted on every machine: WSL receives the dotfiles themselves through it (`_meta/notes/wsl-init-tuto.md`, line 14), and §4 installs it by hand, key included, before `stow bin` can put `syncthing-update` on the PATH. On any machine where the script runs, Syncthing and its keyring already exist (`sys-update` walkthrough, 2026-09-18)

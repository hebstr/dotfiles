...
**A structure can be described correctly and rendered flat.** `gtsum_format()` flattened a `tbl_hierarchical()` table (the adaptation is in `rules/r.md`), while `x$table_styling$indent` still looked exactly like the working version: the styling frame accumulates entries, and which one wins is decided at render.
...
**Two other defects only the image showed.** A header number breaking across two lines (`N = 93 908` cut after `93`) once a fourth column narrowed its column, and the empty `by` level column that `rules/r.md` describes, which `tbl_format()`'s `zero_replace` does not clean.
...
- When a formatting helper from the local stack is applied to a gtsummary class it was not written for, expect it to touch the body as well as the header, and check the body specifically.

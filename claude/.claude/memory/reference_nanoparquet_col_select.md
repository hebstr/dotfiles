...
Cost to budget for: the projection pushdown is what `col_select` was buying, so a file carrying a large text column is read whole (eds-prise's import parquet: 743 MB on disk, 3 GB in memory). Read it once at the session-setup entry point and derive every frame from it by `select()`, rather than once per consumer script.

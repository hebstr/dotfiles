...
- `bats_require_minimum_version` directives on a suite that uses no version-gated feature (`run -N`, `run !`, and the rest of the Bats 1.5 surface): the directive pins a constraint nothing exercises. 4 of the 23 suites here carry one, so its absence is the house majority, not an oversight
...
One criterion settles the recurring "this abort path is untested" finding, which arrives undifferentiated and is wrong either way in bulk. A command failing under `set -e` deserves a test only when the failure leaves state that a later guard misreads: `rig-update`'s `setup_apt_repo` fetches a GPG key whose next-run guard is `[[ ! -f "$DEST" ]]`, so a transfer cut mid-flight leaves a partial key that the guard accepts as installed, which is a missing rollback path and in scope. Its `do_upgrade` has no such state, the retry being idempotent, so a test there asserts that `set -e` aborts, which is a test of bash. Split the finding on that question before accepting or rejecting it. Note also that the stub cannot validate the remedy (a hand-written `curl` stub does not implement `--remove-on-error`): assert that the script *passes* the flag, which is a differential assertion on the code, rather than that the stub honours it, which is the stub-fidelity trap above.

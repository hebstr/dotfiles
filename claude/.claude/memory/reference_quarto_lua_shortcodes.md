...
description: Missing shortcode attributes arrive as empty Inlines (truthy), so presence tests silently discard fallbacks; Pandoc parses YAML metadata scalars as inline Markdown and silently mutilates Lua patterns; YAML 1.1 booleans reach Lua and crash pandoc.Plain; io.open on a named pipe hangs the render; an unregistered shortcode is re-emitted as literal text, so restricting one to a format belongs in an is_format guard rather than in per-format registration; plus the verified Pandoc filesystem/metadata primitives usable inside a Quarto shortcode
metadata:
  type: reference
---
...
**An unregistered shortcode is re-emitted as literal text, never dropped.** When no handler resolves a `{{< name … >}}` call, Quarto rebuilds the source string from the parsed pieces and puts it back in the document (`/opt/quarto/share/filters/main.lua`, the `handler == nil` branch inside `shortcodes_filter`, read on Quarto 1.10.18).

**Why it matters:** `contributes.formats.<format>.shortcodes` is a real key, resolved exactly like `formats.<format>.filters` (`/opt/quarto/bin/quarto.js`, `formatMeta.shortcodes = (formatMeta.shortcodes || []).flatMap(...)`), so scoping a shortcode to one format looks like the declarative way to make it format-specific. It is a trap: the formats that lose the handler print the raw `{{< … >}}` call into the PDF or the Word file instead of rendering nothing.

**How to apply:** keep the registration at the top level of `contributes:` and put the format decision inside the handler, as an `is_format` guard returning `{}`. Put the guard at the head of the handler, ahead of any file read or `quarto.doc.add_html_dependency` call, so the work is bounded rather than the result hidden. Applied in `quarto-hebstr-doc` `filters/add-code-files.lua` on 2026-09-10, verified on a real Typst render (no listing, no literal call, surrounding prose intact); `filters/filetree.lua` carries the same guard to pick a degraded branch instead of an empty one.

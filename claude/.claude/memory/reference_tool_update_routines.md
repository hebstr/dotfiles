...
- **`uv tool upgrade --all`**: updates everything installed with `uv tool install`, which lands in `~/.local/bin`. Covers on `ju-TP`, as `uv tool list` reads it 2026-09-20: pyrefly, sqlfluff, showboat, ouroboros-ai, ggsql-jupyter, yt-dlp. `huggingface-hub` belongs to the same routine on `ju-TP2`, where the models live (`.claude/DESIGN-GPU-REMOTE.md`), and was uninstalled from `ju-TP` on 2026-09-20 as a leftover of the May 2026 llama.cpp smoke test. `sys-update`'s `uv-tools` module runs it. A tool being Rust-written says nothing here: pyrefly is Rust and belongs to this routine, not the two above. This routine only *updates*; nothing in the repo installs a uv tool on a fresh machine (backlog item in `.claude/DEFERRED.md`, scope = those tools plus the uv-managed interpreters).

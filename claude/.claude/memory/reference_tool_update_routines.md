...
- **`cargo install-update`** updates every crate registered in `~/.cargo/.crates.toml`, which is what `cargo install` writes; a binary dropped into `~/.cargo/bin` by any other means (`arf`, currently) carries no entry and is invisible to it.

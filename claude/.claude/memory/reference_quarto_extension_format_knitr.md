...
**A figure never reaches the page's webfonts, so only a font embedded in the SVG itself makes it self-contained.** Quarto inserts each SVG as `<img src="data:image/svg+xml;...">`, and an `<img>`-loaded SVG is an isolated document that the page's `@font-face` rules do not cross into (verified on `index_stat.html`: 19 `font-family` references, zero `@font-face`, all figures in `<img>`). Figure text resolves against the **reader's** installed fonts. Everything below settles what the SVG *claims* and how wide it lays out, never what a reader sees. Making figures self-contained needs the font embedded in the SVG (`svglite::font_face(..., embed = TRUE)` via `web_fonts`), measured at ~743 KB for one figure in `md-nesrine`, hence rejected there at the time. `quarto-hebstr-doc` has since taken that route: its `fonts/register.R` knitr hook adds `web_fonts` to `dev.args` on every `svglite` chunk, so each figure carries the two embedded faces whatever the render machine has installed (its `.claude/CLAUDE.md`, checked 2026-09-22). svglite's own docs state this obliquely: web fonts mean "viewers of the final SVG will not need the font".
...
**`dev.args` is replaced wholesale, not merged.** Verified with two chunks under a format-level alias: the one setting `dev.args` for another reason fell back to `Liberation Sans`. A chunk needing the transparent background that theme-adaptive figures require (quarto-hebstr-doc `README.md`, `renderings`) must repeat `system_fonts` in the same call.

...
**Why:** `set -e` *does* propagate to assignments via command substitution in modern bash (verified: `bash -c 'set -e; x=$(false); echo y'` exits 1). Combined with `pipefail`, any failing pipeline element kills the parent. `grep` semantically returns 1 on "no match" — fine when grep is a test, fatal when grep is a filter.

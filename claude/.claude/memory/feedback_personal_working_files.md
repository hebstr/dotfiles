...
description: Rationale and edge cases behind the write-gate on NOTES.md, TODO.md and CALENDRIER.md (reading is free; the rule lives in permissions.deny and rules/claude-files.md)
metadata:
  type: feedback
---

The rule is enforced by `permissions.deny` in `settings.json` and stated in `rules/claude-files.md`, section "Content": `NOTES.md`, `TODO.md` and `CALENDRIER.md` are read and grepped like any other file, and never written to. This note carries only what the bullet has no room for.
...
**How to apply, beyond the `rules/claude-files.md` bullet:**

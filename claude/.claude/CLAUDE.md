...
- After completing any non-trivial task: propose adversarial review with `/audit:walkthrough <target>` systematically (adversarial cross-provider validation is on by default). The review must happen in a fresh conversation — never the build conversation; shared context makes the adversary compliant. `/audit:skill-adversary` is reserved for reviewing SKILL.md files only; never propose it for code, tests, or any non-skill artifact.

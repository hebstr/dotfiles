...
For language-specific conventions (idioms and style, formatters, linters, CLI flags, and the per-language lint/format/test gate commands), see `rules/python.md`, `rules/shell.md`, `rules/r.md`, `rules/quarto.md`. These auto-load when editing a matching file; load manually when an R/Python/shell/Quarto task is in play before any file is touched (e.g. an `rv add` decision made in Bash). On any conflict between this file and a `rules/*.md` file on a language-specific point (gate ordering, idiom, flag), the rules file is authoritative for that language.
...
- After writing or modifying code, run the project's lint + format + test gate before reporting the task as done; never wait to be asked. Treat it as part of the task, not as an optional follow-up. Order: follow the per-language sequence in `rules/{shell,python,r}.md` (authoritative for the exact fixer/formatter/linter ordering, which differs per language: Python runs `ruff check --fix` before `ruff format`, shell runs `shellharden` before `shfmt`, R runs `air format` before `jarl`), then tests last. Stop at the first hard failure, fix, then re-run from the top. The exact per-language command sequences (shell, Python, R) live in `rules/{shell,python,r}.md`, auto-loaded on matching files. Quarto's render gate lives in `rules/quarto.md` (auto-loaded on `.qmd` and Quarto project config files: `_quarto.yml`, `_metadata`, `_brand.yml`, ...; see its `paths:` front matter for the exact set). Formats with no rule file:

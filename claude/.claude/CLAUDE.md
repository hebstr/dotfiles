...
For language-specific conventions (idioms and style, formatters, linters, CLI flags, and the per-language lint/format/test gate commands), see `rules/python.md`, `rules/shell.md`, `rules/r.md`. These auto-load when editing a matching file; load manually when an R/Python/shell task is in play before any file is touched (e.g. an `rv add` decision made in Bash). On any conflict between this file and a `rules/*.md` file on a language-specific point (gate ordering, idiom, flag), the rules file is authoritative for that language.

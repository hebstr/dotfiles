...
Secret files contain credentials whose value must never appear in tool output. Any output from a Bash/Read tool lands in the conversation transcript and the local jsonl log under `~/.claude/projects/`; once written there, treating the secret as compromised is the only safe assumption. Scope: `~/.secrets`, any `.env`/`.envrc`, `credentials*`, `.pgpass`, `.netrc`, `*.pem`, `*.key`, `*.pfx`, `id_rsa*`/`id_ed25519*`, plus anything the user flags as containing a secret.

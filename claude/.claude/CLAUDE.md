...
- The source is a git working tree elsewhere on the system (typically under `~/Documents/`, `~/projects/`, or another dev path). Find it via `git remote get-url origin` inside the marketplace clone, then locate the sibling clone of that same remote under `~`. Apply the edit on the source.
...
- Route by target type:
  ...
  - `SKILL.md` files → `/audit:walkthrough <target> --reviewer audit:skill-adversary`, or `/audit:blindspot <target>` for cross-model judging on all findings

...
- `["*","ask","*git mv*","ask"]` : le shell demande confirmation hors de la liste de lecture et du gate, et les dernières règles gardent `git add`, `git commit`, `git rm` et `git mv` sur confirmation (depuis le 2026-09-25, `.claude/DESIGN-OPENCODE-HARNESS.md`, « Git writes: `add`, `commit`, `rm` and `mv` ask, the other verbs are denied, 2026-09-25 »).

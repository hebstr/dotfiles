...
`PROMPT_COMMAND` and `__fix_venv_prompt` stay as they are: the function only assigns `PS1` and prints nothing, so `bleopt prompt_command_changes_layout` is not needed.
